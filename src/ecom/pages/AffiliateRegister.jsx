...
export default function AffiliateRegister() {
  ...
  return (
    <div className="min-h-screen bg-card flex relative overflow-hidden">
      ...
      <div className="hidden lg:flex lg:w-[45%] flex-col justify-between p-10 relative">
        <div className="absolute inset-0 overflow-hidden">
          <div className="absolute top-20 right-10 w-72 h-72 bg-primary/8 rounded-full blur-[100px]" />
          <div className="absolute bottom-20 left-10 w-72 h-72 bg-primary/8 rounded-full blur-[100px]" />
        </div>
        <div className="relative">
          <Link to="/ecom" className="group flex items-center gap-3">
            <img src="/logo.png" alt="Scalor" className="h-10 object-contain" />
            <div>
              <span className="text-[9px] font-medium text-muted-foreground uppercase tracking-[0.2em]">{tp('Programme d\'affiliation')}</span>
            </div>
          </Link>
        </div>
        <div className="relative">
        ...
        <div className="relative flex items-center gap-3">
          ...
          <span className="text-xs text-muted-foreground">{tp('Sans engagement')}</span>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center py-8 px-6 sm:px-10 lg:px-20">
        <div className="w-full max-w-md mx-auto">
          ...
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-2xl font-bold text-foreground">{tp('Rejoindre le programme affilié')}</h1>
            <p className="mt-1 text-muted-foreground text-sm">{tp('Recevez votre lien, suivez vos ventes et vos gains.')}</p>
          </div>
          ...
          <div className="bg-card border border-border rounded-2xl p-6 sm:p-7 shadow-xl">
            ...
            <div className="flex bg-muted rounded-xl p-1 mb-5">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-medium transition-all ${
                  mode === 'scalor' ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'
                }`}
              >
            ...
            {mode === 'scalor' && (
              <>
                <p className="text-xs text-muted-foreground mb-4 bg-primary-50 border border-primary-100 rounded-lg px-3 py-2">
                ...
                <form className="space-y-4" onSubmit={submitScalor}>
                  <div>
                    <label className="block text-sm font-medium text-foreground mb-1.5">{tp('Email Scalor')}</label>
                    <div className="relative">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-foreground mb-1.5">{tp('Mot de passe Scalor')}</label>
                    <div className="relative">
                      <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground">
                      ...
                      <input value={scalorForm.password} onChange={(e) => setScalorForm(f => ({ ...f, password: e.target.value }))} type={showPassword ? 'text' : 'password'} required placeholder="••••••••" autoComplete="current-password"
                        className="block w-full pl-10 pr-10 py-3 bg-card border border-gray-300 rounded-xl text-foreground placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent text-sm transition" />
            ...
            {mode === 'new' && (
              <form className="space-y-4" onSubmit={submitNew}>
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1.5">{tp('Numéro Mobile Money')}</label>
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground">
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1.5">{tp('Mot de passe')}</label>
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
                    </span>
          ...
          <div className="flex items-center gap-3 my-5">
            <div className="flex-1 h-px bg-gray-200"></div>
            <span className="text-xs text-muted-foreground">{tp('Déjà affilié ?')}</span>
            <div className="flex-1 h-px bg-gray-200"></div>
          </div>

          <Link
            to="/affiliate/login"
            className="w-full px-4 py-3 bg-background hover:bg-muted border border-border hover:border-gray-300 rounded-xl text-sm font-medium text-foreground transition text-center flex items-center justify-center gap-2"
          >
            {tp('Se connecter')}
          </Link>
          ...
          <div className="mt-6 flex items-center justify-center gap-3 text-xs text-muted-foreground">
            ...
            <span>•</span>
            <Link to="/ecom/privacy" className="text-muted-foreground hover:text-foreground transition">{tp('Confidentialité')}</Link>
          </div>
        </div>
      </div>
    </div>
