      ...
      <div className="p-4 md:p-6 max-w-5xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="bg-card border border-border rounded-xl p-4">
            ...
            <p className="text-[11px] uppercase tracking-wide text-muted-foreground mt-0.5">{tp('Total commissions')}</p>
          </div>
          <div className="bg-card border border-border rounded-xl p-4">
            ...
            <p className="text-[11px] uppercase tracking-wide text-muted-foreground mt-0.5">{tp('En attente')}</p>
          </div>
          <div className="bg-card border border-border rounded-xl p-4">
            <div className="w-8 h-8 bg-primary-50 text-primary rounded-lg flex items-center justify-center mb-2">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
            </div>
            <p className="text-xl font-bold text-primary">{fmt(kpis?.approvedCommissions)} <span className="text-sm font-medium">F</span></p>
            <p className="text-[11px] uppercase tracking-wide text-muted-foreground mt-0.5">{tp('Approuvées')}</p>
          </div>
          <div className="bg-card border border-border rounded-xl p-4">
            ...
            <p className="text-[11px] uppercase tracking-wide text-muted-foreground mt-0.5">{tp('Payées')}</p>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-r from-[#0F6B4F] to-[#0a5040] rounded-xl p-5 text-white">
          <div className="flex items-center justify-between">
            ...
            <div className="w-12 h-12 bg-card/10 rounded-xl flex items-center justify-center">
        ...
        <div className="bg-card border border-border rounded-xl">
          <div className="px-4 py-3 border-b border-border">
            <h3 className="text-sm font-semibold text-foreground">{tp('Historique mensuel')}</h3>
          </div>
          {months.length > 0 ? (
            <div className="divide-y divide-gray-100">
              {months.map(m => (
                <div key={m.key} className="p-4 flex flex-col sm:flex-row sm:items-center gap-3 hover:bg-background transition-colors">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-foreground capitalize">{m.label}</p>
                    <p className="text-xs text-muted-foreground">{m.count} conversion{m.count > 1 ? 's' : ''}</p>
                  </div>
            ...
            <p className="text-sm text-muted-foreground text-center py-8">{tp('Aucune commission enregistrée.')}</p>
          )}
        </div>
        ...
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-border">
            <h3 className="text-sm font-semibold text-foreground">{tp('Détail des commissions')}</h3>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-background border-b border-border">
                  ...
                  <th className="px-4 py-3 text-right text-xs font-semibold text-muted-foreground uppercase tracking-wider">{tp('Vente')}</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-muted-foreground uppercase tracking-wider">{tp('Type')}</th>
              ...
              <tbody className="divide-y divide-gray-100">
                ...
                {conversions.length === 0 && (
                  <tr><td colSpan={6} className="px-4 py-8 text-center text-sm text-muted-foreground">{tp('Aucune commission enregistrée.')}</td></tr>
                )}
              </tbody>
            </table>
          </div>
