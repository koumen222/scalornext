...
const UserSupport = () => {
  ...
  return (
    <div className="h-full flex flex-col bg-background">
      ...
      {/* Toast */}
      {toast && (
        <div className={`fixed top-4 right-4 z-50 px-4 py-2.5 rounded-xl shadow-lg text-sm font-semibold text-white flex items-center gap-2 ${toast.type === 'error' ? 'bg-red-500' : 'bg-primary'}`}>
          {toast.type === 'error'
      ...
      <div className="flex items-center justify-between px-4 sm:px-6 py-4 bg-card border-b border-border">
        <div className="flex items-center gap-3">
          {mobileView === 'detail' && (
            <button onClick={() => setMobileView('list')} className="md:hidden p-1.5 text-muted-foreground hover:text-foreground hover:bg-muted rounded-lg transition">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">{tp('Soumettez vos problèmes et suivez leur avancement')}</p>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="bg-card rounded-2xl shadow-2xl w-full max-w-lg">
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              <h2 className="text-base font-bold text-foreground">{tp('Nouveau ticket de support')}</h2>
              <button onClick={() => setShowNew(false)} className="p-1.5 text-muted-foreground hover:text-muted-foreground hover:bg-muted rounded-lg transition">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
            <form onSubmit={createTicket} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-semibold text-foreground mb-1.5">{tp('Sujet')}</label>
                <input type="text" value={newSubject} onChange={e => setNewSubject(e.target.value)}
                  placeholder={tp('Résumez votre problème en quelques mots…')}
                  className="w-full px-3 py-2.5 border border-border rounded-xl text-sm focus:ring-2 focus:ring-[#0F6B4F] focus:border-transparent outline-none bg-background focus:bg-card transition"
                  maxLength={200} />
              </div>
              <div>
                <label className="block text-sm font-semibold text-foreground mb-1.5">{tp('Catégorie')}</label>
                <div className="grid grid-cols-3 gap-2">
                  {CATEGORIES.map(c => (
                    <button key={c.value} type="button" onClick={() => setNewCategory(c.value)}
                      className={`px-3 py-2 text-xs font-semibold rounded-xl border transition-all ${
                        newCategory === c.value ? 'bg-[#0F6B4F] text-white border-[#0F6B4F]' : 'bg-card text-muted-foreground border-border hover:border-[#0F6B4F]/40'
                      }`}>
                      {c.label}
                    </button>
                  ))}
              ...
              <div>
                <label className="block text-sm font-semibold text-foreground mb-1.5">{tp('Message')} <span className="text-red-500">*</span></label>
                <textarea value={newMessage} onChange={e => setNewMessage(e.target.value)}
                  placeholder={tp('Décrivez votre problème en détail. Plus vous êtes précis, plus vite nous pourrons vous aider.')}
                  rows={5}
                  className="w-full px-3 py-2.5 border border-border rounded-xl text-sm focus:ring-2 focus:ring-[#0F6B4F] focus:border-transparent outline-none resize-none bg-background focus:bg-card transition"
                  maxLength={2000} required />
                <p className="text-xs text-muted-foreground mt-1 text-right">{newMessage.length}/2000</p>
              </div>
              <div className="flex justify-end gap-3 pt-1">
                <button type="button" onClick={() => setShowNew(false)}
                  className="px-4 py-2 text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-muted rounded-xl transition">
                  {tp('Annuler')}
                </button>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* ── Ticket list ── */}
        <div className={`w-full md:w-80 lg:w-96 border-r border-border flex flex-col bg-card ${mobileView === 'detail' ? 'hidden md:flex' : 'flex'}`}>
            ...
            {[
              ...
              <button key={f.key} onClick={() => setFilter(f.key)}
                className={`flex-1 py-1.5 text-xs font-semibold rounded-lg transition ${
                  filter === f.key ? 'bg-[#0F6B4F] text-white' : 'text-muted-foreground hover:bg-muted'
                }`}>
                {f.label}
              </button>
            ))}
          ...
          {loading ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground px-6 text-center">
              ...
              <p className="text-sm font-medium text-muted-foreground">{tp('Aucun ticket')}</p>
              ...
              {filteredTickets.map(t => {
                ...
                return (
                  <button key={t.sessionId} onClick={() => selectTicket(t)}
                    className={`w-full text-left px-4 py-3.5 transition-colors ${active ? 'bg-[#E6F2ED]' : 'hover:bg-background'}`}>
                    <div className="flex items-start justify-between gap-2 mb-1">
                      <span className={`text-sm font-semibold truncate flex-1 ${active ? 'text-[#0F6B4F]' : 'text-foreground'}`}>
                        {t.subject || tp('Demande support')}
                      </span>
                      <span className="text-[10px] text-muted-foreground whitespace-nowrap">{fmtAgo(t.lastMessageAt)}</span>
                    </div>
                    {t.lastMessage && (
                      <p className="text-xs text-muted-foreground truncate mb-1.5">{t.lastMessage}</p>
                    )}
        ...
        {/* ── Detail pane ── */}
        <div className={`flex-1 flex flex-col bg-card ${mobileView === 'list' ? 'hidden md:flex' : 'flex'}`}>
          {!selected ? (
            <div className="flex-1 flex flex-col items-center justify-center text-gray-300 px-8 text-center">
              ...
              <p className="text-sm font-medium text-muted-foreground">{tp('Sélectionnez un ticket')}</p>
              <p className="text-xs text-muted-foreground mt-1">{tp('ou créez-en un nouveau')}</p>
            </div>
            ...
            <>
              ...
              <div className="px-5 py-4 border-b border-border bg-background">
                <h2 className="text-sm font-bold text-foreground mb-1.5">{detail.subject || tp('Sans objet')}</h2>
                <div className="flex items-center flex-wrap gap-2">
                  ...
                  <span className="text-[10px] text-muted-foreground">Créé le {fmtFull(detail.createdAt)}</span>
                </div>
              </div>
                    ...
                    <div key={m._id || i} className={`flex items-end gap-2 ${isUser ? 'justify-end' : 'justify-start'}`}>
                      ...
                      <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 ${
                        isUser
                          ...
                          : m.senderType === 'ai'
                            ? 'bg-card border border-sky-200 text-foreground rounded-bl-sm'
                        ...
                        <div className={`flex items-center gap-2 mt-1.5 ${isUser ? 'justify-end' : 'justify-start'}`}>
                          <span className={`text-[10px] ${isUser ? 'text-primary-200' : 'text-muted-foreground'}`}>{fmtFull(m.createdAt)}</span>
                        </div>
                      </div>
                    </div>
                  );
              ...
              {detail.workflowStatus !== 'resolved' ? (
                <form onSubmit={sendReply} className="px-5 py-3.5 border-t border-border bg-card">
                  <div className="flex items-end gap-2">
                    <textarea ref={replyRef} value={reply}
                      ...
                      rows={2}
                      className="flex-1 px-3 py-2 border border-border rounded-xl text-sm focus:ring-2 focus:ring-[#0F6B4F] focus:border-transparent outline-none resize-none bg-background focus:bg-card transition"
                      maxLength={2000}
                    />
                ...
                <div className="px-5 py-4 border-t border-border bg-card text-center">
                  <p className="text-sm text-muted-foreground mb-2">{tp('Ce ticket est résolu.')}</p>
                  <button onClick={() => setShowNew(true)}
