...
const eventStyles = {
  ...
  removed: 'bg-background text-muted-foreground border-border',
  ...
  info: 'bg-background text-muted-foreground border-border',
};
...
function Stat({ label, value, detail, tone = 'gray', icon: Icon = Activity }) {
  const tones = {
    gray: 'bg-background text-foreground border-border',
...
function CopyButton({ value }) {
  ...
  return (
    <button
      type="button"
      onClick={copy}
      className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-border bg-card text-muted-foreground transition hover:bg-background hover:text-foreground"
  ...
  if (!events.length) {
    return (
      <div className="px-4 py-10 text-center">
        ...
        <p className="mt-2 text-sm font-medium text-muted-foreground">{tp('Aucun log a afficher')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-left text-sm">
        <thead className="border-b border-border bg-background text-xs font-semibold uppercase text-muted-foreground">
          <tr>
        ...
        <tbody className="divide-y divide-gray-100">
          {events.map((event, index) => (
            <tr key={`${event.id}-${index}`} className="align-top hover:bg-background">
              ...
              <td className="min-w-[320px] px-4 py-3">
                <p className="font-semibold text-foreground">{event.title || tp('Information technique')}</p>
                <p className="mt-1 text-sm leading-5 text-muted-foreground">{event.summary || event.message}</p>
                {event.action && (
                  <p className="mt-2 rounded-lg bg-background px-2.5 py-2 text-xs font-medium leading-5 text-foreground">
                    A faire : {event.action}
                  </p>
                )}
              </td>
              <td className="min-w-[220px] px-4 py-3 text-xs text-muted-foreground">
                <p><span className="font-semibold text-muted-foreground">{tp('De :')}</span> {event.from || event.saslUser || '-'}</p>
                <p className="mt-1"><span className="font-semibold text-muted-foreground">{tp('Vers :')}</span> {event.to || '-'}</p>
                <p className="mt-1 text-muted-foreground">{formatDate(event.timestamp)}</p>
              </td>
              <td className="whitespace-nowrap px-4 py-3 font-mono text-xs text-muted-foreground">{event.queueId || '-'}</td>
              <td className="min-w-[360px] px-4 py-3">
                <div className="flex items-start gap-2">
                  ...
                  <p className="font-mono text-xs leading-5 text-muted-foreground">{event.message}</p>
                </div>
              </td>
            </tr>
          ))}
  ...
  if (!entries.length) {
    return (
      <div className="px-4 py-10 text-center">
        ...
        <p className="mt-2 text-sm font-medium text-muted-foreground">{tp('Aucun envoi journalise pour le moment')}</p>
        <p className="mt-1 text-xs text-muted-foreground">{tp('Chaque email envoye par l\'application apparaitra ici (OTP, notifications, campagnes...).')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-left text-sm">
        ...
        <tbody className="divide-y divide-gray-100">
          {entries.map((entry) => (
            <tr key={entry.id} className="align-top hover:bg-background">
              <td className="whitespace-nowrap px-4 py-3">
                <span className={cn(
                  'inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-semibold',
                ...
                {entry.durationMs > 0 && <p className="mt-1 text-[11px] text-muted-foreground">{entry.durationMs} ms</p>}
              </td>
              <td className="min-w-[180px] px-4 py-3">
                <p className="font-semibold text-foreground">{entry.to}</p>
                <p className="mt-0.5 text-xs text-muted-foreground">{entry.from}</p>
              </td>
              <td className="min-w-[220px] max-w-[320px] px-4 py-3 text-foreground">
                <p className="line-clamp-2">{entry.subject || '(sans sujet)'}</p>
                {entry.meta?.campaignName && (
                  <p className="mt-0.5 text-xs text-muted-foreground">Campagne : {entry.meta.campaignName}</p>
                )}
              </td>
              <td className="whitespace-nowrap px-4 py-3">
                <span className="inline-flex rounded-full border border-border bg-background px-2 py-0.5 text-[11px] font-semibold text-muted-foreground">
                  {sendSourceLabels[entry.source] || entry.source}
                </span>
              </td>
              <td className="whitespace-nowrap px-4 py-3 font-mono text-xs text-muted-foreground">{entry.queueId || '-'}</td>
              <td className="min-w-[240px] max-w-[360px] px-4 py-3">
                {entry.error ? (
                  <p className="font-mono text-xs leading-5 text-red-600">{entry.error}</p>
                ) : (
                  <p className="font-mono text-xs leading-5 text-muted-foreground">{entry.smtpResponse || '-'}</p>
                )}
              </td>
              <td className="whitespace-nowrap px-4 py-3 text-xs text-muted-foreground">{formatDate(entry.sentAt)}</td>
            </tr>
          ))}
        </tbody>
      </table>
    ...
    <div className="mx-auto max-w-7xl p-4 sm:p-6">
        ...
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">{tp('SMTP transactionnel, DKIM, queue, logs et diagnostics DNS.')}</p>
            </div>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            className={cn(
              ...
              autoRefresh ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-border bg-card text-muted-foreground hover:bg-background'
            )}
          >
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  activeTab === tab.id ? 'bg-gray-900 text-white' : 'text-muted-foreground hover:bg-background hover:text-foreground'
                )}
              >
      ...
      {loading && !overview ? (
        <div className="rounded-lg border border-border bg-card p-10 text-center">
          ...
          <p className="mt-3 text-sm font-medium text-muted-foreground">{tp('Chargement du monitoring mail...')}</p>
        </div>
      ) : null}
      ...
      {!loading && overview && activeTab === 'overview' && (
        <div className="grid gap-5 lg:grid-cols-[1fr_360px]">
          <div className="space-y-5">
            <Panel>
              <div className="border-b border-border px-4 py-3">
                <h2 className="font-semibold text-foreground">{tp('Etat des services')}</h2>
              </div>
              <div className="grid gap-3 p-4 sm:grid-cols-2 lg:grid-cols-4">
                {(overview.services || []).map((service) => (
                  <div key={service.name} className="rounded-lg border border-border bg-background p-3">
                    <div className="flex items-center justify-between">
                      <p className="font-mono text-sm font-semibold text-foreground">{service.name}</p>
                      <Pill ok={service.ok} label={service.active} />
                    </div>
                    <p className="mt-2 text-xs text-muted-foreground">enabled: {service.enabled}</p>
                  </div>
                ))}
              </div>
            </Panel>
            ...
            <Panel>
              <div className="flex flex-col gap-3 border-b border-border px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                <h2 className="font-semibold text-foreground">{tp('Derniers evenements mail expliques')}</h2>
                <button
                  type="button"
                  onClick={() => setActiveTab('logs')}
                  className="inline-flex h-9 items-center justify-center rounded-lg border border-border bg-card px-3 text-sm font-semibold text-foreground transition hover:bg-background"
                >
          ...
          <div className="space-y-5">
            <Panel>
              <div className="border-b border-border px-4 py-3">
                <h2 className="font-semibold text-foreground">{tp('Configuration SMTP app')}</h2>
              </div>
              <div className="space-y-3 p-4 text-sm">
                {[
                  ...
                  <div key={key} className="flex items-center justify-between gap-3 rounded-lg bg-background px-3 py-2">
            ...
            <Panel>
              <div className="border-b border-border px-4 py-3">
                <h2 className="font-semibold text-foreground">{tp('Actions rapides')}</h2>
              </div>
              <div className="grid gap-2 p-4">
                <button
                  ...
                  className="inline-flex h-10 items-center justify-center gap-2 rounded-lg border border-border bg-card px-3 text-sm font-semibold text-foreground transition hover:bg-background disabled:opacity-60"
                >
            ...
            <Panel>
              <div className="border-b border-border px-4 py-3">
                <h2 className="font-semibold text-foreground">{tp('Queue actuelle')}</h2>
              </div>
              <div className="p-4">
        ...
        <div className="space-y-4">
          ...
          <Panel>
            <div className="flex flex-col gap-3 border-b border-border px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h2 className="font-semibold text-foreground">{tp('Journal des envois de l\'application')}</h2>
                <p className="text-xs text-muted-foreground">{tp('Chaque email envoye par Scalor (OTP, notifications, campagnes) avec son ID de queue Postfix — croisable avec l\'onglet Logs.')}</p>
              </div>
              <div className="flex flex-col gap-2 sm:flex-row">
                <select
                  value={sendsStatus}
                  onChange={(e) => setSendsStatus(e.target.value)}
                  className="h-10 rounded-lg border border-border px-3 text-sm outline-none focus:border-gray-400"
                >
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="h-10 w-full rounded-lg border border-border pl-9 pr-3 text-sm outline-none focus:border-gray-400 sm:w-64"
                  />
                </div>
                <button
                  type="button"
      ...
      {overview && activeTab === 'logs' && (
        <div className="space-y-4">
          <Panel>
            <div className="flex flex-col gap-3 border-b border-border px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h2 className="font-semibold text-foreground">{tp('Logs Postfix / OpenDKIM')}</h2>
                <p className="text-xs text-muted-foreground">{tp('Chaque ligne est traduite en langage simple. Le detail technique reste disponible pour tracer un ID de queue.')}</p>
              </div>
              <div className="flex flex-col gap-2 sm:flex-row">
                <div className="relative">
                  <Search className="pointer-events-none absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" />
                  <input
                    ...
                    className="h-10 w-full rounded-lg border border-border pl-9 pr-3 text-sm outline-none focus:border-gray-400 sm:w-72"
                  />
                </div>
                <button
                  type="button"
          ...
          {rawLogs && (
            <Panel>
              <div className="flex items-center justify-between border-b border-border px-4 py-3">
                <h2 className="font-semibold text-foreground">{tp('Log brut')}</h2>
      ...
      {overview && activeTab === 'mailbox' && (
        <Panel>
          <div className="border-b border-border px-4 py-3">
            <h2 className="font-semibold text-foreground">{tp('Mails locaux recus')}</h2>
            <p className="text-xs text-muted-foreground">Lecture de `/var/mail/root`. Les emails transactionnels sortants ne sont pas stockes ici sauf livraison locale ou bounce.</p>
          </div>
          {(overview.mailbox || []).length === 0 ? (
            <div className="px-4 py-10 text-center">
              <Inbox className="mx-auto h-8 w-8 text-gray-300" />
              <p className="mt-2 text-sm font-medium text-muted-foreground">{tp('Aucun mail local trouve')}</p>
            </div>
          ) : (
            <div className="divide-y divide-gray-100">
              {overview.mailbox.map((message) => (
                <div key={message.id} className="p-4 hover:bg-background">
                  <div className="flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0">
                      ...
                      {message.preview && <p className="mt-2 text-sm leading-6 text-muted-foreground">{message.preview}</p>}
                    </div>
      ...
      {overview && activeTab === 'dns' && (
        <div className="grid gap-5 lg:grid-cols-[1fr_420px]">
          <Panel>
            <div className="border-b border-border px-4 py-3">
              <h2 className="font-semibold text-foreground">{tp('Etat DNS public')}</h2>
            </div>
            <div className="divide-y divide-gray-100">
              {[
          ...
          <Panel>
            <div className="flex items-center justify-between border-b border-border px-4 py-3">
              <h2 className="font-semibold text-foreground">{tp('DKIM a copier')}</h2>
              <CopyButton value={overview.dkimRecord?.value || ''} />
            </div>
            <div className="space-y-3 p-4">
              <div>
                <p className="text-xs font-semibold uppercase text-muted-foreground">{tp('Nom')}</p>
                <p className="mt-1 font-mono text-sm text-foreground">{overview.dkimRecord?.name}</p>
              </div>
              <div>
                <p className="text-xs font-semibold uppercase text-muted-foreground">{tp('Valeur TXT')}</p>
      ...
      {overview && activeTab === 'config' && (
        <Panel>
          <div className="flex items-center justify-between border-b border-border px-4 py-3">
            <div>
              <h2 className="font-semibold text-foreground">{tp('Configuration Postfix active')}</h2>
              <p className="text-xs text-muted-foreground">Sortie de `postconf -n`.</p>
            </div>
      ...
      {overview && activeTab === 'test' && (
        <div className="grid gap-5 lg:grid-cols-[440px_1fr]">
          <Panel>
            <div className="border-b border-border px-4 py-3">
              <h2 className="font-semibold text-foreground">{tp('Envoyer un mail de test')}</h2>
              <p className="text-xs text-muted-foreground">Utilise `smtpuser` en local sur le port 587 avec STARTTLS.</p>
            </div>
            <form onSubmit={sendTest} className="space-y-4 p-4">
              <label className="block">
                <span className="text-sm font-semibold text-foreground">{tp('Adresse destinataire')}</span>
                <input
                  type="email"
                  required
                  value={testEmail}
                  ...
                  className="mt-1 h-11 w-full rounded-lg border border-border px-3 text-sm outline-none focus:border-gray-400"
                />
              </label>
              <button
                type="submit"
          ...
          <Panel>
            <div className="border-b border-border px-4 py-3">
              <h2 className="font-semibold text-foreground">{tp('Resultat du test')}</h2>
            </div>
            {testOutput ? (
              ...
              <div className="px-4 py-10 text-center">
                ...
                <p className="mt-2 text-sm font-medium text-muted-foreground">{tp('Aucun test lance dans cette session')}</p>
              </div>
            )}
          </Panel>
        </div>
