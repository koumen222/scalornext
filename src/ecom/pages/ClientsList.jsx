...
const CARD = 'bg-card rounded-2xl ring-1 ring-gray-100 shadow-[0_1px_2px_rgba(16,24,40,0.04),0_4px_16px_-8px_rgba(16,24,40,0.06)]';
...
const SC = {
  ...
  confirmed: 'bg-primary-50 text-primary ring-1 ring-primary-100',
  delivered: 'bg-primary text-white',
  returned: 'bg-gray-900 text-white',
  blocked: 'bg-gray-900 text-white',
};
const SDOT = {
  prospect: 'bg-gray-300', confirmed: 'bg-primary-400', delivered: 'bg-primary', returned: 'bg-gray-800', blocked: 'bg-gray-900',
};
...
const selectCls = 'h-10 px-3 bg-background/70 rounded-xl text-sm text-muted-foreground ring-1 ring-gray-100 focus:outline-none focus:ring-2 focus:ring-primary-300 focus:bg-card cursor-pointer transition-all';
...
const ClientsList = () => {
  ...
  const availableSyncStatuses = [
    { key: 'delivered', get label() { return tp('Livré'); }, dot: 'bg-primary', clientStatus: 'Client' },
  ...
  const summary = [
    ...
    { label: 'livrés', value: stats.delivered || 0, dot: 'bg-primary' },
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="p-3 sm:p-4 lg:p-6 max-w-6xl mx-auto">
        {success && (
          <div className="mb-4 flex items-center gap-2.5 p-3 bg-primary-50 text-primary rounded-2xl text-sm ring-1 ring-primary-100">
        ...
        <div className="flex items-center justify-between gap-3 mb-4">
          <div className="flex items-center gap-2.5">
            <h1 className="text-xl font-bold text-foreground">{tp('Clients')}</h1>
            {autoSyncing && (
          ...
          <div className="flex items-center gap-2">
            {user?.role === 'ecom_admin' && (
              <button
                ...
                disabled={syncing}
                className="h-9 px-3 bg-card ring-1 ring-gray-200 text-foreground rounded-xl text-sm font-medium flex items-center gap-2 hover:bg-background hover:ring-gray-300 transition-all disabled:opacity-50"
              >
                {syncing ? (
        ...
        <div className={`${CARD} px-4 py-3 mb-3 flex items-center gap-x-4 gap-y-1 flex-wrap`}>
          <span className="text-sm font-semibold text-foreground">{stats.total || 0} {tp('clients')}</span>
          <span className="hidden sm:block w-px h-4 bg-muted"/>
          {summary.map(s => (
            <span key={s.label} className="inline-flex items-center gap-1.5 text-sm text-muted-foreground">
              <span className={`w-1.5 h-1.5 rounded-full ${s.dot}`}/>
              <span className="font-medium text-foreground">{s.value}</span> {s.label}
            </span>
          ))}
          {user?.role === 'ecom_admin' && (stats.total || 0) > 0 && (
            <button onClick={handleDeleteAll} disabled={deletingAll} className="ml-auto text-xs font-medium text-muted-foreground hover:text-foreground transition-colors disabled:opacity-50">
        ...
        <div className={`${CARD} overflow-hidden`}>
          ...
          <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-50">
            <svg className="w-4 h-4 text-muted-foreground flex-shrink-0 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-4.35-4.35M10.5 18a7.5 7.5 0 100-15 7.5 7.5 0 000 15z"/></svg>
            <input
              type="text"
              placeholder={tp('Rechercher des clients')}
              value={search}
              onChange={e => setSearch(e.target.value)}
              className="flex-1 min-w-0 h-9 bg-transparent text-sm text-foreground placeholder:text-muted-foreground focus:outline-none"
            />
            <button
              onClick={() => setShowFilters(!showFilters)}
              className={`h-9 px-3 rounded-lg text-sm font-medium flex items-center gap-1.5 transition-all flex-shrink-0 ${showFilters || activeFilters.length > 0 ? 'bg-primary-50 text-primary ring-1 ring-primary-100' : 'text-muted-foreground hover:bg-background'}`}
            >
              ...
              {activeFilters.length > 0 && <span className="w-4 h-4 bg-primary text-white rounded-full text-[10px] font-bold flex items-center justify-center">{activeFilters.length}</span>}
            </button>
          </div>

          {/* Filtres dépliables */}
          {showFilters && (
            <div className="px-3 py-2.5 border-b border-gray-50 bg-background/40 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
          ...
          {activeFilters.length > 0 && (
            <div className="flex items-center flex-wrap gap-1.5 px-3 py-2 border-b border-gray-50">
              {activeFilters.map(f => (
                <button key={f.key} onClick={f.clear} className="inline-flex items-center gap-1 h-6 px-2 bg-background text-muted-foreground ring-1 ring-gray-100 rounded-full text-xs font-medium hover:bg-muted transition-colors">
                  {f.label} <span className="text-muted-foreground">×</span>
                </button>
              ))}
              <button onClick={resetAllFilters} className="text-xs font-medium text-muted-foreground hover:text-foreground ml-1">{tp('Tout effacer')}</button>
            </div>
          )}
          ...
          {loading ? (
            <div className="divide-y divide-gray-50">
              {Array.from({ length: 8 }).map((_, i) => (
                <div key={i} className="flex items-center gap-3 px-4 py-3.5 animate-pulse">
                  ...
                  <div className="hidden lg:block h-3 bg-muted rounded w-28 ml-auto"/>
                  <div className="hidden lg:block h-3 bg-muted rounded w-24"/>
                </div>
              ))}
            </div>
          ) : clients.length === 0 ? (
            <div className="py-16 text-center px-6">
              ...
              <p className="text-sm font-semibold text-foreground mb-1">{activeFilters.length > 0 || search ? 'Aucun résultat' : tp('Aucun client')}</p>
              <p className="text-sm text-muted-foreground mb-4">{activeFilters.length > 0 || search ? 'Modifie les filtres pour élargir la recherche.' : tp('Ajoute un premier client pour démarrer.')}</p>
              {(activeFilters.length > 0 || search) && (
                <button onClick={resetAllFilters} className="h-9 px-4 ring-1 ring-gray-200 rounded-xl text-sm text-muted-foreground hover:bg-background transition-colors">{tp('Effacer les filtres')}</button>
              )}
            </div>
          ) : (
                ...
                <thead>
                  <tr className="border-b border-gray-50">
                    {[['Nom du client', 'text-left'], ['Téléphone', 'text-left'], ['Emplacement', 'text-left'], ['Commandes', 'text-right'], ['Montant dépensé', 'text-right']].map(([h, cls]) => (
                      <th key={h} className={`px-4 py-2.5 text-[11px] font-semibold text-muted-foreground uppercase tracking-wide ${cls}`}>{tp(h)}</th>
                    ))}
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            <span className={`inline-flex text-[10px] font-semibold px-2 py-0.5 rounded-full flex-shrink-0 ${SC[c.status] || 'bg-muted text-muted-foreground'}`}>
                        ...
                        {/* Téléphone */}
                        <td className="px-4 py-3 text-sm text-muted-foreground truncate">{c.phone || <span className="text-gray-300">—</span>}</td>

                        {/* Emplacement */}
                        <td className="px-4 py-3 text-sm text-muted-foreground truncate">{location || <span className="text-gray-300">—</span>}</td>

                        {/* Commandes */}
                        <td className="px-4 py-3 text-sm text-muted-foreground text-right tabular-nums">{c.totalOrders || 0}</td>

                        {/* Montant dépensé */}
                        <td className="px-4 py-3 text-sm font-medium text-foreground text-right tabular-nums whitespace-nowrap">{fmtF(c.totalSpent)}</td>
                        ...
                        <td className="px-2 py-3 text-right">
                          {user?.role === 'ecom_admin' ? (
                            <button
                              ...
                              className="h-7 w-7 inline-flex items-center justify-center text-gray-300 rounded-lg hover:bg-muted hover:text-foreground transition-colors opacity-0 group-hover:opacity-100"
                              title={tp('Supprimer')}
                            >
      ...
      {showSyncModal && (
        <div className="fixed inset-0 bg-gray-900/40 backdrop-blur-sm flex items-end sm:items-center justify-center z-50" onClick={() => setShowSyncModal(false)}>
          <div className="bg-card rounded-t-3xl sm:rounded-3xl shadow-xl max-w-md w-full" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-5 pt-4 pb-7 sm:pb-5">
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  <h3 className="text-sm font-bold text-foreground">{tp('Synchroniser les clients')}</h3>
                  <p className="text-xs text-muted-foreground">{tp('Importer les contacts depuis les commandes')}</p>
                </div>
              </div>

              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">{tp('Statuts à synchroniser')}</p>
              <div className="space-y-1.5 mb-5 max-h-60 overflow-y-auto">
                {availableSyncStatuses.map(s => (
                  <label key={s.key} className={`flex items-center gap-3 h-10 px-3 rounded-xl ring-1 cursor-pointer transition-colors ${syncStatuses.includes(s.key) ? 'ring-primary-300 bg-primary-50/50' : 'ring-gray-100 hover:bg-background'}`}>
                    <input
                      type="checkbox"
                    ...
                    <span className="text-xs text-muted-foreground">→ {s.clientStatus}</span>
                  </label>
                ))}
              </div>

              <div className="flex gap-2.5">
                <button onClick={() => setShowSyncModal(false)} className="flex-1 h-11 bg-muted text-muted-foreground rounded-2xl text-sm font-semibold hover:bg-gray-200 transition-colors">{tp('Annuler')}</button>
                <button
                  onClick={handleSyncClients}
                  disabled={syncing || syncStatuses.length === 0}
                  className="flex-1 h-11 bg-primary text-white rounded-2xl text-sm font-semibold hover:bg-primary-700 disabled:opacity-40 flex items-center justify-center gap-2 transition-colors"
                >
