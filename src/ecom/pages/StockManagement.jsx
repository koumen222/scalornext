    ...
    <div className="space-y-4">
      ...
      <div className="relative bg-white rounded-2xl border border-gray-100 overflow-hidden p-5">
        ...
        <div className="relative flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            ...
            <p className="mt-1 text-sm text-gray-500">{tp('Suivez chaque entrée avec une date visible sur chaque ligne.')}</p>
          </div>
          <button
            onClick={openAdd}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-4 space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">{tp('Stock actuel')}</p>
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">{tp('unités disponibles')}</p>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-4 space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">{tp('Ventes')}</p>
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">{tp('unités vendues')}</p>
          </div>
          <div className="space-y-1">
            <div className="flex justify-between text-[10px] text-gray-400">
        ...
        <div className={`bg-white rounded-2xl border p-4 space-y-3 ${lowStockCount > 0 ? 'border-red-100' : 'border-gray-100'}`}>
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">{tp('Ruptures')}</p>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-4 space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">{tp('Lignes')}</p>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-100 py-16 text-center">
          ...
          <p className="text-sm font-medium text-gray-700 mb-1">{tp('Aucune entrée de stock')}</p>
          <p className="text-sm text-gray-400 mb-4">{isFiltered ? 'Essayez d\'élargir vos filtres' : tp('Commencez par ajouter une ligne')}</p>
          {!isFiltered && (
            <button onClick={openAdd} className="text-sm text-primary-600 font-semibold hover:underline">{tp('+ Ajouter une ligne')}</button>
          )}
        </div>
      ) : (
        <>
                ...
                <div key={entry._id} className="bg-white rounded-2xl border border-gray-100 p-4">
                  ...
                  <div className="grid grid-cols-3 gap-2 text-center mb-3">
                    <div className="bg-gray-50 rounded-xl p-2">
                      <p className="text-[9px] text-gray-400 uppercase font-semibold mb-0.5">{tp('Initial')}</p>
                    ...
                    <div className={`rounded-xl p-2 ${stockActuel <= 5 ? 'bg-red-50' : stockActuel <= 15 ? 'bg-amber-50' : 'bg-primary-50'}`}>
                      <p className={`text-[9px] uppercase font-semibold mb-0.5 ${stockActuel <= 5 ? 'text-red-500' : stockActuel <= 15 ? 'text-amber-600' : 'text-primary-600'}`}>{tp('Actuel')}</p>
          ...
          <div className="hidden sm:block bg-white rounded-2xl border border-gray-100 overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="px-5 py-3.5 text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide">{tp('Date')}</th>
                  <th className="px-5 py-3.5 text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide">{tp('Produit')}</th>
                  <th className="px-5 py-3.5 text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide">{tp('Ville')}</th>
                  <th className="px-5 py-3.5 text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide">{tp('Agence')}</th>
                  <th className="px-5 py-3.5 text-right text-[11px] font-semibold text-gray-400 uppercase tracking-wide">{tp('Initial')}</th>
                  <th className="px-5 py-3.5 text-right text-[11px] font-semibold text-gray-400 uppercase tracking-wide">{tp('Ventes')}</th>
                  <th className="px-5 py-3.5 text-right text-[11px] font-semibold text-gray-400 uppercase tracking-wide">{tp('Actuel')}</th>
                  <th className="px-5 py-3.5 text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide">{tp('État')}</th>
            ...
            <div className="px-5 py-3 bg-gray-50/50 border-t border-gray-100 flex items-center justify-between">
              ...
              <div className="flex items-center gap-5 text-xs">
                <span className="text-gray-500">{tp('Stock total :')} <strong className="text-gray-800 tabular-nums">{totalStock.toLocaleString('fr-FR')}</strong></span>
                <span className="text-gray-500">{tp('Ventes :')} <strong className="text-emerald-600 tabular-nums">{totalSales.toLocaleString('fr-FR')}</strong></span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={e => { if (e.target === e.currentTarget) closeModal(); }}>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h2 className="text-base font-bold text-gray-900">{editingId ? 'Modifier la ligne' : tp('Nouvelle ligne de stock')}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  <label className="block text-xs font-semibold text-gray-700 mb-1">{tp('Produit *')}</label>
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">{tp('Date *')}</label>
                  <input type="date" value={form.stockDate} onChange={e => setForm(p => ({ ...p, stockDate: e.target.value }))}
                    className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-primary-500 outline-none bg-gray-50" required />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">{tp('Ville')}</label>
                  <input type="text" value={form.city} onChange={e => setForm(p => ({ ...p, city: e.target.value }))}
                    className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-primary-500 outline-none bg-gray-50" placeholder={tp('Ex: Douala')} />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">{tp('Agence')}</label>
                  <input type="text" value={form.agency} onChange={e => setForm(p => ({ ...p, agency: e.target.value }))}
                    className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-primary-500 outline-none bg-gray-50" placeholder={tp('Ex: Lygos, Anka')} />
                </div>
                <div>
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">{tp('Ventes')}</label>
              ...
              {form.quantity !== '' && (
                <div className="rounded-xl p-3.5 bg-primary-50 border border-primary-100 flex items-center justify-between">
                  <span className="text-sm text-primary-700 font-medium">{tp('Stock actuel calculé')}</span>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-700 mb-1">{tp('Notes')}</label>
                <input type="text" value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))}
                  className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-primary-500 outline-none bg-gray-50" placeholder={tp('Notes optionnelles...')} />
              </div>
