    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-muted-foreground font-medium">{tp('Chargement des statistiques...')}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-background p-6">
        <div className="max-w-2xl mx-auto">
          <div className="bg-red-50 border border-red-200 rounded-xl p-6 text-center">
            ...
            <h3 className="text-lg font-bold text-red-900 mb-2">{tp('Erreur de chargement')}</h3>
    ...
    <div className="min-h-screen bg-background/50 pb-20 lg:pb-8">
      ...
      <div className="bg-card border-b border-border sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-5">
          <div className="flex items-center gap-4">
            <button onClick={() => navigate('/ecom/sourcing')} aria-label={tp('Retour au sourcing')} className="p-2 -ml-2 rounded-lg hover:bg-muted active:scale-95 text-muted-foreground hover:text-foreground transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-400 focus-visible:ring-offset-1">
              <Ico d={I.back} className="w-5 h-5" aria-hidden="true" />
            </button>
            <div>
              <h1 className="text-xl sm:text-2xl font-black text-foreground tracking-tight flex items-center gap-2">
                <Ico d={I.chart} className="w-6 h-6 text-primary" />
                {tp('Statistiques Sourcing')}
              </h1>
              <p className="text-sm text-muted-foreground mt-1 font-medium">{tp('Vue d\'ensemble complète des commandes et paiements')}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <section aria-labelledby="overview-title">
          <h2 id="overview-title" className="text-base font-bold text-foreground mb-3 flex items-center gap-2">
            <Ico d={I.package} className="w-5 h-5 text-muted-foreground" aria-hidden="true" />
            Vue d'ensemble
          </h2>
        ...
        <section aria-labelledby="toplan-title">
          <h2 id="toplan-title" className="text-base font-bold text-foreground mb-3 flex items-center gap-2">
            <Ico d={I.alert} className="w-5 h-5 text-orange-500" aria-hidden="true" />
            {tp('Montant à prévoir')}
            <span className="text-sm font-medium text-muted-foreground">{tp('(commandes en transit, impayées)')}</span>
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="bg-card border border-border rounded-2xl p-5 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">{tp('Chine')}</p>
              ...
              <p className="text-2xl font-black text-foreground tabular-nums mb-3">{formatMoney(toPlan.china.total)}</p>
              <div className="space-y-1.5 pt-3 border-t border-border">
                <div className="flex justify-between text-xs font-medium">
                  <span className="text-muted-foreground">{tp('Achat')}</span>
                  <span className="text-foreground tabular-nums">{formatMoney(toPlan.china.purchase)}</span>
                </div>
                <div className="flex justify-between text-xs font-medium">
                  <span className="text-muted-foreground">{tp('Transport')}</span>
                  <span className="text-foreground tabular-nums">{formatMoney(toPlan.china.transport)}</span>
                </div>
                <div className="flex justify-between text-xs font-medium pt-1 border-t border-border">
                  <span className="text-muted-foreground">{toPlan.china.orders} commande{toPlan.china.orders > 1 ? 's' : ''}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-card border border-border rounded-2xl p-5 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">{tp('Local')}</p>
              ...
              <p className="text-2xl font-black text-foreground tabular-nums mb-3">{formatMoney(toPlan.local.total)}</p>
              <div className="pt-3 border-t border-border">
                <span className="text-xs font-medium text-muted-foreground">{toPlan.local.orders} commande{toPlan.local.orders > 1 ? 's' : ''}</span>
              </div>
            </div>

            <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-2xl p-5 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <p className="text-xs font-semibold text-purple-100 uppercase tracking-wide">{tp('Total à prévoir')}</p>
                <div className="w-7 h-7 rounded-lg bg-card/20 flex items-center justify-center" aria-hidden="true">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Chine */}
          <section aria-labelledby="china-title" className="bg-card rounded-2xl border p-6 shadow-sm space-y-5">
            <h2 id="china-title" className="text-base font-bold text-foreground flex items-center gap-2">
              {tp('Paiements Chine')}
              <span className="ml-auto text-xs font-semibold text-muted-foreground bg-muted px-2 py-1 rounded-full">{payment.china.total} commandes</span>
            </h2>
            ...
            <div className="space-y-3 pt-4 border-t border-border">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">{tp('Achat Chine')}</p>
              ...
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide pt-2">{tp('Transport')}</p>
          ...
          {/* Local */}
          <section aria-labelledby="local-title" className="bg-card rounded-2xl border p-6 shadow-sm space-y-5">
            <h2 id="local-title" className="text-base font-bold text-foreground flex items-center gap-2">
              {tp('Paiements Local')}
              <span className="ml-auto text-xs font-semibold text-muted-foreground bg-muted px-2 py-1 rounded-full">{payment.local.total} commandes</span>
            </h2>
            <div className="space-y-2">
            ...
            <div className="space-y-3 pt-4 border-t border-border">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">{tp('Montants')}</p>
        ...
        <section aria-labelledby="products-title">
          <h2 id="products-title" className="text-base font-bold text-foreground mb-3 flex items-center gap-2">
            <Ico d={I.package} className="w-5 h-5 text-muted-foreground" aria-hidden="true" />
            {tp('Statistiques Produits')}
          </h2>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          {products.lowStockProducts && products.lowStockProducts.length > 0 && (
            <div className="mt-4 bg-orange-50 border border-orange-200 rounded-2xl p-4">
              <h3 className="font-bold text-orange-900 mb-3 flex items-center gap-2 text-sm">
                ...
                {tp('Produits en stock faible')}
              </h3>
              <div className="space-y-2">
                {products.lowStockProducts.map(p => (
                  <div key={p._id} className="flex justify-between items-center text-sm bg-card rounded-xl p-3 border border-orange-100">
        ...
        <section aria-labelledby="financial-title">
          <h2 id="financial-title" className="text-base font-bold text-foreground mb-3 flex items-center gap-2">
            <Ico d={I.cash} className="w-5 h-5 text-muted-foreground" aria-hidden="true" />
            {tp('Résumé Financier')}
          </h2>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
...
function StatCard({ icon, label, value, color }) {
  const cfg = {
    blue:    { bg: 'bg-card border-border', badge: 'bg-blue-100', icon: 'text-blue-600' },
    emerald: { bg: 'bg-card border-border', badge: 'bg-primary-100', icon: 'text-primary' },
    orange:  { bg: 'bg-card border-border', badge: 'bg-orange-100', icon: 'text-orange-600' },
    red:     { bg: 'bg-card border-border', badge: 'bg-red-100', icon: 'text-red-600' },
    purple:  { bg: 'bg-card border-border', badge: 'bg-purple-100', icon: 'text-purple-600' },
    yellow:  { bg: 'bg-card border-border', badge: 'bg-yellow-100', icon: 'text-yellow-600' },
  };
...
function AmountRow({ label, value, color }) {
  const textColor = color === 'emerald' ? 'text-primary' : color === 'red' ? 'text-red-600' : 'text-foreground';
  return (
...
function PaymentStatusRow({ label, value, total, color }) {
  ...
  const textColor = { emerald: 'text-primary', yellow: 'text-amber-700', red: 'text-rose-700' };

  return (
    <div className="py-1.5">
      <div className="flex justify-between items-center mb-1.5">
        <span className="text-sm font-medium text-foreground">{label}</span>
        <span className={`text-xs font-bold tabular-nums ${textColor[color]}`}>{value} <span className="text-muted-foreground font-normal">({percentage}%)</span></span>
      </div>
      <div
