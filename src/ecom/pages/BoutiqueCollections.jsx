...
const inputCls = 'w-full px-3 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-600 focus:border-transparent transition bg-gray-50 focus:bg-white';
    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/60 px-4 py-6" onClick={() => !saving && onClose()}>
      <div className="flex max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
          <h3 className="text-base font-black text-gray-900">{editing ? tp('Modifier la collection') : tp('Nouvelle collection')}</h3>
          <button onClick={onClose} className="rounded-full p-1.5 text-gray-400 transition hover:bg-gray-100 hover:text-gray-700"><X className="h-4 w-4" /></button>
        </div>

        <div className="flex-1 space-y-4 overflow-y-auto px-5 py-4">
          <div>
            <label className="mb-1 block text-xs font-semibold text-gray-600">{tp('Nom de la collection')}</label>
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold text-gray-600">{tp('Description (optionnelle)')}</label>
          ...
          {/* Image de couverture */}
          <div>
            <label className="mb-1 block text-xs font-semibold text-gray-600">{tp('Image de couverture')}</label>
            <div className="flex items-center gap-3">
              <div className="flex h-16 w-24 flex-shrink-0 items-center justify-center overflow-hidden rounded-xl border border-gray-200 bg-gray-50">
                {form.image ? <img src={form.image} alt="" className="h-full w-full object-cover" /> : <ImageIcon className="h-5 w-5 text-gray-300" />}
              </div>
              <label className="inline-flex cursor-pointer items-center gap-1.5 rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-xs font-bold text-gray-600 transition hover:bg-gray-100">
          ...
          <div>
            <div className="mb-2 flex items-center justify-between">
              <label className="text-xs font-semibold text-gray-600">{tp('Produits de la collection')} <span className="font-black text-primary-700">({form.productIds.length})</span></label>
              <div className="relative">
                <Search className="absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-gray-400" />
                <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={tp('Rechercher…')} className="w-44 rounded-lg border border-gray-200 bg-gray-50 py-1.5 pl-8 pr-2 text-xs outline-none focus:border-primary-500" />
              </div>
            </div>
            <div className="max-h-64 space-y-1 overflow-y-auto rounded-xl border border-gray-100 p-2">
              {filteredProducts.length === 0 && (
                <p className="px-2 py-4 text-center text-xs text-gray-400">{tp('Aucun produit trouvé')}</p>
              )}
              {filteredProducts.map((p) => {
                ...
                return (
                  <button
                    ...
                    className={`flex w-full items-center gap-2.5 rounded-lg border p-2 text-left transition ${isSel ? 'border-primary-300 bg-primary-50/60' : 'border-transparent hover:bg-gray-50'}`}
                  >
                    <span className={`flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-md border-2 transition ${isSel ? 'border-primary-600 bg-primary-600 text-white' : 'border-gray-300 bg-white'}`}>
                      {isSel && <Check className="h-3 w-3" strokeWidth={3} />}
                    </span>
                    {p.image ? <img src={p.image} alt="" className="h-8 w-8 flex-shrink-0 rounded-lg object-cover" /> : <div className="h-8 w-8 flex-shrink-0 rounded-lg bg-gray-100" />}
                    <span className="min-w-0 flex-1 truncate text-[13px] font-semibold text-gray-800">{p.name}</span>
          ...
          <label className="flex cursor-pointer items-center gap-2">
            <input type="checkbox" checked={form.enabled} onChange={(e) => setForm((f) => ({ ...f, enabled: e.target.checked }))} className="h-4 w-4 rounded border-gray-300 text-primary-600" />
            <span className="text-sm font-medium text-gray-700">{tp('Collection visible sur la boutique')}</span>
          </label>
        ...
        <div className="flex justify-end gap-2 border-t border-gray-100 px-5 py-3.5">
          <button onClick={onClose} disabled={saving} className="rounded-xl bg-gray-100 px-4 py-2.5 text-sm font-bold text-gray-600 transition hover:bg-gray-200 disabled:opacity-50">{tp('Annuler')}</button>
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6">
      <div className="mb-6 flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h1 className="flex items-center gap-2 text-xl font-black text-gray-900"><Layers className="h-5 w-5 text-primary-700" /> {tp('Collections')}</h1>
          <p className="mt-0.5 text-sm text-gray-500">{tp('Regroupez vos produits (page publique /collections/nom) — comme sur Shopify.')}</p>
        </div>
      ...
      {collections.length === 0 ? (
        <div className="rounded-2xl border-2 border-dashed border-gray-200 bg-white p-14 text-center">
          ...
          <p className="text-sm font-bold text-gray-600">{tp('Aucune collection pour le moment')}</p>
          <p className="mx-auto mt-1 max-w-sm text-xs text-gray-400">{tp('Créez par exemple « Nouveautés », « Meilleures ventes » ou « Soins visage » et ajoutez-y vos produits.')}</p>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {collections.map((c) => (
            <div key={c._id} className={`overflow-hidden rounded-2xl border bg-white shadow-sm transition ${c.enabled !== false ? 'border-gray-200' : 'border-gray-200 opacity-60'}`}>
              <div className="flex h-28 items-center justify-center bg-gray-50" style={c.image ? { backgroundImage: `url(${c.image})`, backgroundSize: 'cover', backgroundPosition: 'center' } : {}}>
                {!c.image && <Layers className="h-7 w-7 text-gray-200" />}
              </div>
              <div className="p-4">
                <div className="flex items-center justify-between gap-2">
                  <p className="truncate text-sm font-black text-gray-900">{c.name}</p>
                  <span className="flex-shrink-0 rounded-full bg-gray-100 px-2 py-0.5 text-[10.5px] font-bold text-gray-600">{c.productCount ?? (c.productIds || []).length} {tp('produits')}</span>
                </div>
                <p className="mt-0.5 truncate text-xs text-gray-400">/collections/{c.slug}</p>
                <div className="mt-3 flex items-center gap-1">
                  <button onClick={() => setModal(c)} className="inline-flex items-center gap-1 rounded-lg bg-gray-100 px-2.5 py-1.5 text-[11.5px] font-bold text-gray-700 transition hover:bg-gray-200">
                    <Pencil className="h-3 w-3" /> {tp('Modifier')}
                  </button>
                  <button onClick={() => toggleEnabled(c)} title={c.enabled !== false ? tp('Masquer') : tp('Afficher')} className="rounded-lg p-1.5 text-gray-400 transition hover:bg-gray-100 hover:text-gray-700">
                    {c.enabled !== false ? <Eye className="h-3.5 w-3.5" /> : <EyeOff className="h-3.5 w-3.5" />}
                  </button>
                  <button onClick={() => remove(c)} title={tp('Supprimer')} className="rounded-lg p-1.5 text-gray-400 transition hover:bg-red-50 hover:text-red-500">
                    <Trash2 className="h-3.5 w-3.5" />
                  </button>
                  <div className="flex-1" />
                  {storefrontBase && c.enabled !== false && (
                    <a href={`${storefrontBase}/collections/${c.slug}`} target="_blank" rel="noopener noreferrer" title={tp('Voir sur la boutique')} className="rounded-lg p-1.5 text-gray-400 transition hover:bg-gray-100 hover:text-primary-700">
