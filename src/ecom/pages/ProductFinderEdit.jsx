...
const KpiPill = ({ label, value, color = 'text-foreground' }) => (
...
const ProductFinderEdit = () => {
  ...
  if (loading) return (
    <div className="p-6 max-w-2xl mx-auto space-y-3">
      {[...Array(6)].map((_,i) => <div key={i} className="h-12 bg-muted rounded-2xl animate-pulse" />)}
    </div>
  );
      ...
      <div className="bg-card border-b border-border px-4 sm:px-6 py-4 sticky top-0 z-10">
        <div className="max-w-2xl mx-auto flex items-center gap-3">
          ...
          <div className="min-w-0">
            <h1 className="text-sm font-bold text-foreground truncate">{tp('Modifier le produit')}</h1>
            <p className="text-[11px] text-muted-foreground">{tp('Mise à jour de la veille')}</p>
          </div>
