...
const AffiliateLayoutComponent = ({ children, affiliate }) => {
  ...
  const NavLink = ({ item }) => {
    ...
    return (
      ...
      >
        <span className={`flex-shrink-0 ${active ? 'text-[#0F6B4F]' : 'text-muted-foreground group-hover:text-muted-foreground'}`}>
          {item.icon}
        </span>
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col lg:flex-row overflow-x-hidden max-w-[100vw]">
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex lg:flex-col lg:w-[220px] lg:fixed lg:inset-y-0 z-30 bg-card border-r border-border">
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 px-3 py-3 space-y-0.5 overflow-y-auto">
            <p className="px-2 pt-1 pb-1.5 text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{tp('Navigation')}</p>
            ...
            <div className="px-3 pb-3 pt-2">
              ...
              >
                ...
                <div className="min-w-0 flex-1 text-left">
                  <p className="text-xs font-semibold text-foreground truncate">{affiliate?.name || tp('Affilié')}</p>
                  <p className="text-[10px] text-muted-foreground truncate">{affiliate?.email || ''}</p>
                </div>
                <svg className="w-4 h-4 text-muted-foreground flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 lg:ml-[220px]">
        {/* Mobile Header */}
        <header className="lg:hidden fixed top-0 left-0 right-0 z-20 bg-card border-b border-border">
        ...
        {/* Desktop Header */}
        <header className="hidden lg:flex border-b h-14 items-center px-5 fixed top-0 left-[220px] right-0 z-20 bg-card border-border gap-4">
          <div className="flex items-center gap-2 min-w-[160px]">
            <span className="text-xs text-muted-foreground">{tp('Code :')} <span className="font-mono font-bold text-[#0F6B4F]">{affiliate?.referralCode || '—'}</span></span>
          </div>
          <div className="flex-1" />
          <div className="flex items-center gap-3 flex-shrink-0">
            <button
              onClick={handleLogout}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm text-muted-foreground hover:bg-muted hover:text-foreground transition-colors"
            >
        ...
        <div className="flex items-stretch px-2" style={{ height: '60px' }}>
          {mobileMainTabs.map((item) => {
            ...
            return (
              ...
              >
                <span className={`transition-colors duration-200 ${active ? 'text-[#0F6B4F]' : 'text-muted-foreground'}`}>
                  {mobileIcon(item)}
                </span>
                <span className={`text-[10px] font-medium ${active ? 'text-[#0F6B4F]' : 'text-muted-foreground'}`}>
                  {item.name}
                </span>
              </Link>
            );
