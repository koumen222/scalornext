    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-gradient-to-br from-violet-600 to-indigo-700 px-4 pt-12 pb-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            onClick={loadStats}
            className="w-9 h-9 rounded-full bg-card/20 flex items-center justify-center text-white active:scale-95"
          >
        ...
        {loading ? (
          <div className="h-20 bg-card/10 rounded-2xl animate-pulse" />
        ) : (
          <div className="bg-card/10 backdrop-blur-sm rounded-2xl p-5">
          ...
          <>
            ...
            <div className="bg-card rounded-2xl border shadow-sm overflow-hidden">
              <div className="px-4 py-3 border-b border-border">
                <h2 className="font-semibold text-foreground">{tp('Résumé')}</h2>
              </div>
...
function PeriodCard({ title, icon, amount, deliveries, bg }) {
  return (
    <div className={`${bg} rounded-2xl p-4`}>
      ...
      <p className="text-xs text-muted-foreground mt-0.5">{deliveries} livraison{deliveries > 1 ? 's' : ''}</p>
    </div>
  );
}
...
function SummaryRow({ label, value, icon, highlight }) {
  return (
    <div className={`flex items-center gap-3 px-4 py-3 ${highlight ? 'bg-green-50/50' : ''}`}>
      {icon}
      <span className={`flex-1 text-sm ${highlight ? 'font-semibold text-foreground' : 'text-muted-foreground'}`}>
        {label}
      </span>
      <span className={`font-bold text-base ${highlight ? 'text-green-600' : 'text-foreground'}`}>
        {value}
      </span>
    </div>
  );
